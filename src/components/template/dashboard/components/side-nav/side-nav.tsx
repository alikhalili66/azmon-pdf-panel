...
type SideNavProps = Props_Block & { routes: any[] };
...
export const SideNav = ({
	...
	const Li = ({ item, isSubItem = false, boxClass = '' }) => {
		...
		return (
			<>
				<li
					className={`${boxClass} sm-parent-hover flex min-h-[50px] relative rounded-tr-[25px] rounded-br-[25px] cursor-pointer items-center justify-center gap-[10px] mt-[15px] py-[5px] ${
						isActiveItem ? 'bg-background-primary' : ''
					} ${collapseNav ? '' : 'lg:justify-start lg:pr-[5px]'}`}
					onClick={() =>
				...
				>
					{item?.awesomeIcon && (
						...
						>
							<i className={`fa fa-${item.awesomeIcon} !text-[22px] ${collapseNav ? 'sm-child-hidden' : ''} `} />
						</span>
					)}
					...
					<span
						className={`hidden ${
							collapseNav
								? 'sm-child-inline-flex sm:items-center sm:justify-center sm:text-center sm:absolute sm:top-0 sm:right-0 sm:left-0 sm:bottom-0 sm:text-[13px]'
								: 'lg:inline-block'
						}`}
					>
						{item?.name || ''}
	...
	return (
		<Block boxClass={`${boxClass} select-none [&>*]:transition-none hidden md:block `} boxSize={boxSize} boxSpace={boxSpace}>
			<ul className={`overflow-hidden pr-[2px] ${collapseNav ? '' : 'lg:border-t-[5px] lg:border-background-primary'}`}>
				{routes.map(
					(item: any, i) =>
						item?.inSideNav && (

		...
		return array.filter((item) => {
			...
			if (selectMethod === 'OR') {
				const findedItem = (propertiesArray || []).find((propertyItem) => {
					...
					const formattedValue = Convert.arabicCharToFa(caseSensitive ? String(value || '').toLowerCase() : String(value || ''));

					const formattedItemValue = Convert.arabicCharToFa(
						caseSensitive ? String(item[keyName] || '').toLowerCase() : String(item[keyName] || ''),
					);
